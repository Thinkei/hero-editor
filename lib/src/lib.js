...
const HeroEditor = ({
  ...
  const wrapperSizes = useRef({ width: 0, height: 0 });
  ...
  useEffect(() => {
    ...
    if (
      width !== wrapperSizes.current.width ||
      height !== wrapperSizes.current.height
    ) {
      wrapperSizes.current = { width, height };
      onResize(wrapperSizes.current);
      postMessage(EDITOR_RESIZE, wrapperSizes.current, editor);
    }
  }, [value]);

...
const handleMessage = (editor) => {
  ...
  const removeMentionApplyListener = addMessageListener(
    'mention-apply',
    ({ editor, data }) => {
      const { id, name, style, meta, target } = data;
      Transforms.select(editor, target);
      const mention = {
        type: MENTION,
        data: { id, name, style, meta },
        children: [{ text: '' }],
      };
...
const MentionListWrapper = ({ renderMentionList }) => {
  ...
  return (
    ...
    >
      {renderMentionList(search, ({ id, name, style, meta }) => {
        postMessage('mention-apply', { id, name, style, meta, target }, editor);
      })}
    </div>
  );
};
